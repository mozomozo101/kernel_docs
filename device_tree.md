...
## デバイスをprobeするまでの流れ
1. bootloaderが、r1レジスタにマシンIDを入れ、kernel_entry()を呼ぶ
2. kernelが起動し、init_machine() が呼ばれる
3. ボードファイルは、デバイスや、ボード固有の情報を登録するために、SoC用のAPIを呼ぶ
4. SoCファイルは、ピンマルチ設定したり、デバイスをplatformバスに登録する。（デバイスのベースアドレスやIRQ番号も）。（ここよく分からない）
5. ドライバがprobe()を呼ぶ

※ ドライバは、early_param() とかを使って、起動初期段階で登録しておくんだと思う。
